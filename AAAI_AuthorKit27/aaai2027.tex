...
\begin{abstract}
	Speech LLM-based ASR has achieved strong general recognition performance, but remains vulnerable to named entities and long-tail words due to dominant language model priors. Retrieval augmented contextual ASR mitigates this issue by injecting candidate hotwords as prompts, yet its effectiveness critically depends on accurately retrieving short textual entities from long speech utterances under weak supervision. Existing speech-text retrievers often rely on global utterance embeddings or unconstrained local matching, which suffer from representation dilution and alignment drift. We propose a token-level aligned late-interaction retriever for weakly supervised hotword retrieval. The proposed method converts frame-level speech representations into monotonic acoustic tokens, and performs length-aware sliding-window matching between acoustic tokens and hotword text tokens. This design preserves fine-grained entity cues while allowing flexible local alignment without timestamp annotations. We further introduce token-level auxiliary alignment and false-negative-aware batch construction to improve contrastive learning reliability. Experiments on contextual ASR benchmarks show that our method substantially improves hotword retrieval accuracy and reduces both CER and biased-word error rate when used as a retrieval front-end for Speech LLM prompting.
\end{abstract}
...
We construct the training data from large-scale Chinese and English speech corpora. For Chinese (ZH), we use AISHELL-1(100h), AISHELL-2 (1000+h), and WenetSpeech(10,000+h) . For English (ZH+EN), we use LibriSpeech (960h) and GigaSpeech(10,000+h). Based on these corpora, we consider two training regimes. The \textit{ZH} setting uses only the AISHELL-2 corpora, whereas the \textit{ZH+EN} setting additionally incorporates all corpora. This comparison allows us to evaluate both in-language retrieval performance and the robustness of different matching mechanisms under restricted language coverage.

For each training utterance, candidate hotwords are randomly extracted from its transcription. We ensure that the evaluation utterances and hotword candidates do not overlap with the training set at the utterance level.

\paragraph{Evaluation Benchmarks.}
We evaluate all systems on Test-AISHELL1-NE, ContextASR-Bench and AISpeechMeeting. Test-AISHELL1-NE mainly evaluates Chinese named-entity and hotword retrieval under relatively clean read-speech conditions. ContextASR-Bench contains both Chinese and English evaluation subsets and is used to assess contextual speech retrieval across different languages and acoustic conditions. We additionally evaluate on AISpeech-Meeting to measure cross-domain generalization in real word meeting speech, where background noise, speaker variation, and conversational speaking styles make local hotword evidence more difficult to identify.


\paragraph{Model Configuration.}
We use Paraformer as the speech encoder and \texttt{bert-base-multilingual-uncased} as the text encoder for all compared methods. Paraformer converts the input speech into a sequence of acoustic representations, while the multilingual BERT encoder produces contextualized representations for the candidate hotword tokens. The speech and text representations are projected into a shared $d$-dimensional embedding space through modality-specific projection layers.

To ensure a controlled comparison, GLCLAP, CLAR, and TLDR use the same speech encoder, text encoder, training data, projection dimension, and optimization configuration. They differ only in the speech-text interaction mechanism. GLCLAP uses global and local pooled representations, CLAR performs localized speech-text matching, and TLDR applies token-level late interaction over CIF-derived acoustic tokens.

Unless otherwise specified, the Paraformer and BERT parameters are fine-tuned jointly, and all projection and matching modules are optimized end to end.

\paragraph{Evaluation Metrics.}
We evaluate the systems from two complementary perspectives: hotword detection and candidate ranking. For threshold-based detection, we report precision, recall, and F1 score. For ranking, we report Recall@K. For downstream contextual ASR, we report hotword F1 and word error rate (WER).

\paragraph{Implementation Details.}
All models are implemented in PyTorch and trained using Adamw with an initial learning rate of 1e-5. We use a batch size of 128 and train each model for EN+ZH 10epochs and ZH 30 epochs. The acoustic and text representations are projected to a shared dimension of d=512. The temperature parameter in the contrastive objective is set to $1 / 0.07$, and the maximum candidate hotword length is 256 tokens.

For TLDR, the CIF module converts frame-level speech representations into monotonic acoustic tokens. Candidate dependent local windows are constructed according to the number of text tokens in each hotword. We use acoustic token frames length as the slide window step and mean pool to aggregate token-level matching scores.

All reported results are averaged over 3 random seeds.  Training is conducted on 32 Nvidia A800 GPUs.

\input{tldr_results_bundle/results_insert}


\subsection{Mechanism Analysis}

\paragraph{Partial-match discrimination.}

Standard retrieval benchmarks are nearly saturated, making it difficult to
determine whether a model truly distinguishes a complete hotword from a
partially matched alternative. We therefore construct controlled negative
candidates that share most tokens with the target hotword and evaluate whether
the positive hotword receives a higher score. As shown in
Figure~\ref{fig:partial_match}, TLDR remains consistently above the baselines
as the negative candidate becomes increasingly similar to the target. In random
high-overlap cases, CLAR already achieves strong pairwise accuracy, but TLDR
produces a substantially larger positive-negative margin, indicating stronger
separation between complete and partial matches. Under phonetic replacements,
the gap becomes more pronounced: TLDR improves high-overlap PairAcc from
$81.3\%$ to $88.2\%$ over CLAR. These results suggest that token-level late
interaction does not merely improve score calibration; it provides more robust
token-wise acoustic evidence for rejecting partially matched hotwords.

\begin{figure*}[t]
	\centering
	\includegraphics[width=0.5\textwidth]{figure/partial_match_combined_main.pdf}
	\caption{
		Partial-match discrimination under random and phonetic replacements.
		Each row corresponds to one replacement type. The first two columns show
		PairAcc and positive-negative margin as token overlap increases, and the
		last column reports the aggregated high-overlap setting. TLDR maintains
		both higher accuracy and larger margins, especially for phonetic
		replacements, indicating more robust token-wise acoustic evidence.
	}
	\label{fig:partial_match}
\end{figure*}

\subsection{Mechanism Analysis: Token Evidence Removal}
\label{sec:token-evidence-removal}

The partial-match challenge in Section~\ref{sec:partial-match} probes the
text side of hotword retrieval: negative candidates differ from the target
hotword by only a few tokens. We further ask whether the model's score is
supported by the corresponding acoustic evidence in the utterance. To this
end, we introduce a speech-side intervention, \emph{token evidence removal}.
For each hotword $h=(w_1,\ldots,w_m)$ and each constituent token $w_j$, we
construct three versions of the same utterance: the original audio $x$, a
token-masked audio $x_{\mathrm{tok}(j)}$ where only the time span aligned to
$w_j$ is zeroed out, and a background-masked audio $x_{\mathrm{bg}(j)}$ where
an equal-duration non-hotword span from the same utterance is zeroed out. The
hotword query remains unchanged in all three conditions.

Let $s(x,h)$ denote the retrieval score for hotword $h$ on audio $x$. We
measure the token-specific effect by subtracting an equal-duration background
control:
\begin{align}
	\Delta_{\mathrm{raw}}(j)
	 & = s(x,h) - s(x_{\mathrm{tok}(j)}, h),                 \\
	\Delta_{\mathrm{bg}}(j)
	 & = s(x,h) - s(x_{\mathrm{bg}(j)}, h),                  \\
	\Delta_{\mathrm{sel}}(j)
	 & = \Delta_{\mathrm{raw}}(j) - \Delta_{\mathrm{bg}}(j).
\end{align}
We report the mean selective drop and Token Evidence Coverage (TEC),
\begin{equation}
	\mathrm{TEC} = \frac{1}{N}\sum_{i=1}^{N}
	\mathbb{I}\left[\Delta_{\mathrm{sel}}^{(i)} > \epsilon\right],
	\quad \epsilon=0.01,
\end{equation}
where $N$ is the number of evaluated hotword-token instances. This metric
counts the fraction of tokens whose removal causes a score decrease beyond
what is explained by masking an unrelated background region.

For the primary CJK evaluation, token time spans are obtained from FunASR
\texttt{fa-zh}/ASRFA alignments. English is used as a supplementary diagnostic:
we align target references with WhisperX, then split each aligned word span
across its mBERT wordpieces. Since English wordpieces are not guaranteed to
correspond to phonetic boundaries, we use the CJK results for the main
mechanistic claim.

\begin{figure}[t]
	\centering
	\includegraphics[width=\linewidth]{figure/token_evidence_combined_main.pdf}
	\caption{Token evidence removal analysis. The left panel shows the
		distribution of selective score drops after removing aligned token evidence,
		and the right panel reports Token Evidence Coverage (TEC). }
	\label{fig:token-evidence-combined-main}
\end{figure}
...
\begin{table}[t]
	...
	\begin{tabular}{lccc}
		\toprule
		Method         & Tokens & Mean selective drop & TEC    \\
		\midrule
		TLDR           & 6160   & 0.0478              & 0.6364 \\
		Global-Pooling & 6160   & 0.0014              & 0.0471 \\
		CLAR           & 6160   & 0.0152              & 0.4919 \\
		CLALAP         & 6160   & 0.0000              & 0.1531 \\
		\bottomrule
	\end{tabular}
	\caption{CJK token evidence removal results. Each value is computed after
		subtracting the equal-duration background-masking control. TLDR shows the
		largest selective drop and the highest token evidence coverage.}
	\label{tab:token-evidence-cjk}
\end{table}

Figure~\ref{fig:token-evidence-combined-main} visualizes the selective-drop
distribution and TEC across the compared mechanisms. Table~\ref{tab:token-evidence-cjk}
shows a clear separation between TLDR and the baselines. TLDR obtains a mean
selective drop of $0.0478$ and
$\mathrm{TEC}=0.6364$, indicating that removing an aligned constituent token
usually lowers the complete-hotword score more than removing an unrelated
background span. In contrast, Global-Pooling is almost insensitive to this
targeted intervention ($\mathrm{TEC}=0.0471$), which is consistent with a
single utterance-level representation that does not preserve token-local
evidence. CLALAP also shows weak token-specific evidence under this
intervention ($\mathrm{TEC}=0.1531$). CLAR is stronger than the pooled
baselines ($\mathrm{TEC}=0.4919$), but still below TLDR, suggesting that
TLDR's late interaction provides additional coverage over constituent tokens
rather than merely benefiting from general local perturbation sensitivity.

The same ordering holds when the supplementary English set is included. After
filtering hotwords with fewer than two content tokens, the all-dataset TEC
scores are $0.6089$ for TLDR, $0.4613$ for CLAR, $0.0749$ for CLALAP, and
$0.0184$ for Global-Pooling. We therefore use the intervention as evidence
that TLDR's score is more tightly coupled to token-level acoustic evidence.
Importantly, this result does not claim a strict one-to-one alignment between
text tokens and acoustic tokens. Rather, it shows that the model covers more
constituent-token evidence: when a constituent's aligned acoustic span is
removed, TLDR is more likely than the baselines to reduce the score assigned
to the full hotword.
