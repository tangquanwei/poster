...
\newlength{\mainColumn}
\setlength{\mainColumn}{0.303\paperwidth}
...
\begin{tikzpicture}[remember picture,overlay]
  \fill[AppleLight] (current page.north west) rectangle ([yshift=-18.7cm]current page.north east);
  \draw[AppleLine,line width=0.5pt] ([yshift=-18.7cm]current page.north west) -- ([yshift=-18.7cm]current page.north east);
\end{tikzpicture}

\vspace*{-0.7cm}
\begin{columns}[t,totalwidth=\dimexpr\paperwidth-2\posterMargin\relax]
...
\begin{column}{\mainColumn}
  \begin{posterbox}
    ...
    GRGA indexes conversation along semantic, speaker, timestamp, and acoustic dimensions, enabling grounded reasoning over dispersed evidence.}
  \end{posterbox}
...
\begin{column}{\mainColumn}
  \begin{posterbox}
    ...
    \begin{whitebox}
      \centering
      \includegraphics[width=0.99\linewidth]{figure/method.pdf}
    \end{whitebox}
    \vspace{0.2cm}
    \begin{posterlist}
...
\begin{column}{\mainColumn}
  \begin{posterbox}
    ...
    \vspace{0.25cm}
    \begin{whitebox}
      \miniTitle{Future Directions}
      \vspace{0.25cm}
      \smallText\color{AppleGray}
      Reduce iterative latency, strengthen upstream robustness, and extend beyond structured meetings to broader long-form audio.
    \end{whitebox}
  \end{posterbox}
\end{column}
\end{columns}
...
\vfill
\begin{tikzpicture}[remember picture,overlay]
  \node[anchor=south west, xshift=\posterMargin, yshift=1.2cm] at (current page.south west) {
  ...
  \node[anchor=south east, xshift=-\posterMargin, yshift=1.2cm] at (current page.south east) {
    {\fontsize{17}{21}\selectfont\color{AppleGray}
    Code/Data: github.com/tangquanwei/GRGA}
  };
\end{tikzpicture}
